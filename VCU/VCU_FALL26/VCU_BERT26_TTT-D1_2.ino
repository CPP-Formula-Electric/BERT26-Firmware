...
/*
  ...
  Functions:
    ...
    - Logs float telemetry to separate measured (MEASnnnn.BIN), derived
      (DERInnnn.BIN), and per-cell (CELLnnnn.BIN) files on the built-in SD card.
*/
...
// converted and logged directly as floating-point g, deg/s and degC values.
...
// This single switch makes regen easy to disable without changing any logic.
static constexpr bool REGEN_ENABLED = true;
static const float REGEN_MAX_TORQUE_NM = 50.0f;
// Regen reaches its maximum at the end of the light-braking region, then
// tapers back to zero through the heavy-braking region to reduce lockup risk.
static const float REGEN_LIGHT_BRAKE_END_PCT = 25.0f;
static const float REGEN_HEAVY_BRAKE_END_PCT = 40.0f;
// Reduce charging power as SOC approaches full to avoid pack over-voltage.
static const float REGEN_SOC_DERATE_START_PCT = 90.0f;
static const float REGEN_SOC_DISABLE_PCT = 95.0f;
static const float REGEN_MAX_POWER_KW = 20.0f;
// Keep regen below the BMS's 4.200 V/cell hard fault. Regulate toward 4.160 V,
// then latch regen off for 10 seconds if a terminal measurement reaches
// 4.180 V. The two margins prevent normal regulation from repeatedly hitting
// the stop threshold and leave room for quantization and control-loop delay.
static const float REGEN_CELL_OV_LIMIT_V = 4.200f;
static const float REGEN_CELL_CURRENT_TARGET_MARGIN_V = 0.040f;
static const float REGEN_CELL_TERMINAL_STOP_MARGIN_V = 0.020f;
static const float REGEN_CELL_CURRENT_TARGET_V =
    REGEN_CELL_OV_LIMIT_V - REGEN_CELL_CURRENT_TARGET_MARGIN_V;
static const float REGEN_CELL_TERMINAL_STOP_V =
    REGEN_CELL_OV_LIMIT_V - REGEN_CELL_TERMINAL_STOP_MARGIN_V;
// Equivalent DC resistance of one complete 4P cell bank. Pack current flows
// through every series bank, so no series-cell multiplier belongs here.
static const float REGEN_CELL_BANK_RESISTANCE_OHM = 0.00285f;
static const float REGEN_PACK_CURRENT_SENSOR_MAX_A = 500.0f;
static constexpr uint32_t REGEN_VOLTAGE_WINDOW_MS = 10000;
// 0x310 and 0x311 are broadcast every 500 ms. Allow three missed broadcasts,
// then fail safe to zero regen rather than calculating from stale data.
static constexpr uint32_t REGEN_BMS_DATA_TIMEOUT_MS = 1500;
static constexpr uint32_t REGEN_BMS_PAIR_MAX_SKEW_MS = 100;
// A 24-entry ring retains at least 10 seconds at the BMS's 500 ms cadence.
static constexpr uint8_t REGEN_VOLTAGE_WINDOW_CAPACITY = 24;
// The BMS monitors 96 channels, but one is a bypassed busbar channel; the pack
// has 95 active series cell banks.
static constexpr uint8_t REGEN_PACK_SERIES_BANKS = 95;
// Rules cutoff: no regen at or below 5 km/h; ramp to full strength by 15 km/h.
static const float REGEN_CUTOFF_SPEED_KMH = 5.0f;
static const float REGEN_FULL_SPEED_KMH = 15.0f;

// Each entry contains a synchronized 0x310 maximum-cell voltage and 0x311 pack
// current. Pack current is positive while discharging and negative during
// charge/regen, hence Voc ~= Vterminal + Ipack*R.
struct RegenVoltageSampleWindow {
  float terminalV[REGEN_VOLTAGE_WINDOW_CAPACITY];
  float estimatedOpenCircuitV[REGEN_VOLTAGE_WINDOW_CAPACITY];
  uint32_t timestampMs[REGEN_VOLTAGE_WINDOW_CAPACITY];
  uint8_t count;
  uint8_t head;
};

static RegenVoltageSampleWindow regenVoltageWindow = {};
static uint32_t lastRegenVoltageSampleMs = 0;
static bool regenBmsPairValid = false;
...
static constexpr float KMH_PER_MPH           = 1.609344f;
...
static constexpr uint16_t BMS_CELL_COUNT = 96;
static constexpr uint32_t CELL_LOG_REQUEST_PERIOD_MS = 1000;
// A 96-cell voltage dump takes 32 frames * 5 ms in the BMS firmware.
static constexpr uint32_t CELL_VOLTAGE_DUMP_WAIT_MS = 250;
...
// ---------------- Data logging ----------------
// Three binary files are created for each session: MEASnnnn.BIN contains values
// received directly from sensors/CAN, DERInnnn.BIN contains calculated/control
// values, and CELLnnnn.BIN contains every requested cell voltage/temperature.
// Every telemetry field is an IEEE-754 32-bit float; headers use integer framing.
...
static constexpr char MEASURED_LOG_MAGIC[8] = {'V','C','U','M','E','A','S','1'};
static constexpr char DERIVED_LOG_MAGIC[8]  = {'V','C','U','D','E','R','I','1'};
static constexpr char CELL_LOG_MAGIC[8]     = {'V','C','U','C','E','L','L','1'};
...
struct __attribute__((packed)) MeasuredFastRecord {
  float type, timeSeconds;
  float apps1AdcCounts, apps2AdcCounts, bse1AdcCounts, bse2AdcCounts;
  float hvCurrentAdcCounts, motorRpm, busVoltageV, busCurrentA;
};

struct __attribute__((packed)) DerivedFastRecord {
  float type, timeSeconds;
  float brakePressurePct, commandedTorqueNm, regenTorqueNm;
  float speedKmh, cumulativeDistanceKm, statusFlags;
};

struct __attribute__((packed)) MeasuredMediumRecord {
  float type, timeSeconds;
  float glvVoltageV, inverterTempC, motorTempC, socPct, batteryTempC;
  float bmsFlags, bmsFaultCode, imdWarnAlarms, pm100PostFaults, pm100RunFaults;
};

struct __attribute__((packed)) DerivedMediumRecord {
  float type, timeSeconds;
  float apps1Pct, apps2Pct, pedalPct, desiredTorqueNm, powerKw, speedMph;
  float fanDutyPct, rtdState;
};

struct __attribute__((packed)) MeasuredSlowRecord {
  float type, timeSeconds;
  float minCellVoltageV, maxCellVoltageV, avgCellVoltageV;
  float minCellTempC, maxCellTempC, avgCellTempC;
  float imdRIsoKohm, imdRIsoStatus, validFlags;
};

struct __attribute__((packed)) MeasuredImuRecord {
  float type, timeSeconds;
  float accelXG, accelYG, accelZG;
  float gyroXDps, gyroYDps, gyroZDps, temperatureC;
};

struct __attribute__((packed)) CellLogFileHeader {
  char magic[8];
  uint32_t session;
  uint16_t recordSize;
  uint16_t cellCount;
...
// type: 1 = voltage (value in V), 2 = temperature (value in degC).
struct __attribute__((packed)) CellLogRecord {
  float type, timeSeconds, cellIndex, value;
};
...
// kept separate from the diagnostic display request because the bus must stay
// at zero whenever TS/RTD is off.
float commandedTorqueNm = 0.0f;
// Signed torque request shown on the dashboard. It continues to follow the
// pedals (positive drive, negative regen) while TS is off without bypassing
// the safety gate on commandedTorqueNm/pm100CommandTorqueRaw.
float displayedCommandTorqueNm = 0.0f;
...
float bse1Pct = 0.0f;
float bse2Pct = 0.0f;
float brakePressurePct = 0.0f;
bool braking = false;
bool hardBraking = false;
int rawHvCurrentLast = 0;
float regenTorqueNm = 0.0f;
...
// Each log is staged independently in RAM and written in 512-byte chunks.
File sdMeasuredLogFile;
File sdDerivedLogFile;
File sdCellLogFile;
bool sdCardStarted = false;
bool sdLogActive = false;
uint32_t logSessionNumber = 0;
uint8_t measuredLogBuffer[512];
uint8_t derivedLogBuffer[512];
uint8_t cellLogBuffer[512];
size_t measuredLogBufferLen = 0;
size_t derivedLogBufferLen = 0;
size_t cellLogBufferLen = 0;
...
uint32_t lastDistanceUpdateMs = 0;
...
bool haveBmsCurrent = false;
uint32_t lastBmsCellVoltageRxMs = 0;
uint32_t lastBmsCurrentRxMs = 0;
uint8_t lastCellDataRequestType = CELL_DATA_REQUEST_UNKNOWN;
uint32_t lastCellLogRequestMs = 0;
uint32_t cellVoltageRequestMs = 0;
bool cellTemperatureRequestPending = false;
...
void appendSdLog(File &file, bool measuredLog, const void *data, size_t len);
void flushSdLogBuffer(File &file, bool measuredLog);
void appendCellLog(const void *data, size_t len);
void flushCellLogBuffer();
void flushAllSdLogBuffers();
...
void serviceCellDataRequests(uint32_t nowMs);
void writeCellDataResponseLog(const CAN_message_t &msg);
...
float calculateDisplayedCommandTorqueNm();
...
bool pushRegenVoltageSample(uint32_t nowMs);
bool regenVoltageWindowMaximums(uint32_t nowMs, float &maxTerminalV,
                                float &maxEstimatedOpenCircuitV);
float regenCellChargeCurrentLimitA(uint32_t nowMs);
float calculateRegenTorqueNm();
void updateCumulativeDistance(uint32_t nowMs);
...
// Store one voltage/current pair for the regen charge-current model. This is
// called only when the two back-to-back BMS frames arrived close enough to be
// treated as one measurement.
bool pushRegenVoltageSample(uint32_t nowMs) {
  // Reject missing/corrupt BMS voltage statistics. The normal operating range
  // is narrower, but these bounds avoid hiding a genuine high-cell condition.
  if (!haveCellVoltageStats || !haveBmsCurrent ||
      minCellVoltageMv < 1000 || maxCellVoltageMv > 5000 ||
      minCellVoltageMv > avgCellVoltageMv ||
      avgCellVoltageMv > maxCellVoltageMv ||
      fabsf(packCurrent) > REGEN_PACK_CURRENT_SENSOR_MAX_A) {
    return false;
  }

  float terminalV = (float)maxCellVoltageMv / 1000.0f;
  // BMS convention: positive pack current is discharge, negative is charge.
  // Vterminal = Voc - Ipack*R, therefore Voc = Vterminal + Ipack*R.
  float estimatedOpenCircuitV = terminalV +
      packCurrent * REGEN_CELL_BANK_RESISTANCE_OHM;
  if (estimatedOpenCircuitV < 0.0f || estimatedOpenCircuitV > 5.0f) {
    return false;
  }

  uint8_t slot = regenVoltageWindow.head;
  regenVoltageWindow.terminalV[slot] = terminalV;
  regenVoltageWindow.estimatedOpenCircuitV[slot] = estimatedOpenCircuitV;
  regenVoltageWindow.timestampMs[slot] = nowMs;
  regenVoltageWindow.head =
      (uint8_t)((slot + 1u) % REGEN_VOLTAGE_WINDOW_CAPACITY);
  if (regenVoltageWindow.count < REGEN_VOLTAGE_WINDOW_CAPACITY) {
    regenVoltageWindow.count++;
  }
  lastRegenVoltageSampleMs = nowMs;
  return true;
}

// Return the worst terminal and current-corrected maximum-cell voltages still
// inside the 10-second window. Timestamp subtraction remains valid through the
// millis() rollover because all intervals are much shorter than 2^31 ms.
bool regenVoltageWindowMaximums(uint32_t nowMs, float &maxTerminalV,
                                float &maxEstimatedOpenCircuitV) {
  bool found = false;
  for (uint8_t i = 0; i < regenVoltageWindow.count; i++) {
    if ((uint32_t)(nowMs - regenVoltageWindow.timestampMs[i]) >
        REGEN_VOLTAGE_WINDOW_MS) {
      continue;
    }

    float terminalV = regenVoltageWindow.terminalV[i];
    float estimatedOpenCircuitV =
        regenVoltageWindow.estimatedOpenCircuitV[i];
    if (!found || terminalV > maxTerminalV) maxTerminalV = terminalV;
    if (!found || estimatedOpenCircuitV > maxEstimatedOpenCircuitV) {
      maxEstimatedOpenCircuitV = estimatedOpenCircuitV;
    }
    found = true;
  }
  return found;
...
// Absolute allowed charge-current magnitude from the most conservative
// current-corrected cell voltage observed in the last 10 seconds. A recent
// terminal-voltage hit at the 4.180 V stop holds regen at zero for the same
// window, providing hysteresis below the BMS's 4.200 V hard cutoff.
float regenCellChargeCurrentLimitA(uint32_t nowMs) {
  if (!regenBmsPairValid || !haveBmsCurrent || !haveCellVoltageStats ||
      (uint32_t)(nowMs - lastBmsCellVoltageRxMs) > REGEN_BMS_DATA_TIMEOUT_MS ||
      (uint32_t)(nowMs - lastBmsCurrentRxMs) > REGEN_BMS_DATA_TIMEOUT_MS ||
      regenVoltageWindow.count == 0 ||
      (uint32_t)(nowMs - lastRegenVoltageSampleMs) > REGEN_BMS_DATA_TIMEOUT_MS) {
    return 0.0f;
  }

  float maxTerminalV = 0.0f;
  float maxEstimatedOpenCircuitV = 0.0f;
  if (!regenVoltageWindowMaximums(nowMs, maxTerminalV,
                                  maxEstimatedOpenCircuitV)) {
    return 0.0f;
  }

  if (maxTerminalV >= REGEN_CELL_TERMINAL_STOP_V) return 0.0f;

  float voltageHeadroomV =
      REGEN_CELL_CURRENT_TARGET_V - maxEstimatedOpenCircuitV;
  if (voltageHeadroomV <= 0.0f) return 0.0f;
  return voltageHeadroomV / REGEN_CELL_BANK_RESISTANCE_OHM;
}
...
void loop() {
  ...
  speedMph = fabsf((float)motorRpm) * MPH_PER_RPM;
  speedKmh = speedMph * KMH_PER_MPH;
  updateCumulativeDistance(now);
  servicePowerDerate(now);
  updatePm100CommandState();
  servicePm100TxLogging();
  servicePm100BroadcastEnable();
  servicePm100FaultClear();
  serviceCellDataRequests(now);
  serviceDataLogging();
...
void setupDataLogging() {
  ...
  char measuredName[16];
  char derivedName[16];
  char cellName[16];
  snprintf(measuredName, sizeof(measuredName), "MEAS%04lu.BIN", (unsigned long)logSessionNumber);
  snprintf(derivedName, sizeof(derivedName), "DERI%04lu.BIN", (unsigned long)logSessionNumber);
  snprintf(cellName, sizeof(cellName), "CELL%04lu.BIN", (unsigned long)logSessionNumber);

  LogFileHeader measuredHeader;
  memcpy(measuredHeader.magic, MEASURED_LOG_MAGIC, sizeof(measuredHeader.magic));
  measuredHeader.session = logSessionNumber;
  measuredHeader.fastSize = sizeof(MeasuredFastRecord);
  measuredHeader.mediumSize = sizeof(MeasuredMediumRecord);
  measuredHeader.slowSize = sizeof(MeasuredSlowRecord);
  measuredHeader.imuSize = sizeof(MeasuredImuRecord);

  LogFileHeader derivedHeader;
  memcpy(derivedHeader.magic, DERIVED_LOG_MAGIC, sizeof(derivedHeader.magic));
  derivedHeader.session = logSessionNumber;
  derivedHeader.fastSize = sizeof(DerivedFastRecord);
  derivedHeader.mediumSize = sizeof(DerivedMediumRecord);
  derivedHeader.slowSize = 0;
  derivedHeader.imuSize = 0;

  CellLogFileHeader cellHeader;
  memcpy(cellHeader.magic, CELL_LOG_MAGIC, sizeof(cellHeader.magic));
  cellHeader.session = logSessionNumber;
  cellHeader.recordSize = sizeof(CellLogRecord);
  cellHeader.cellCount = BMS_CELL_COUNT;

  sdMeasuredLogFile = SD.open(measuredName, FILE_WRITE);
  sdDerivedLogFile = SD.open(derivedName, FILE_WRITE);
  sdCellLogFile = SD.open(cellName, FILE_WRITE);
  if (sdMeasuredLogFile && sdDerivedLogFile && sdCellLogFile) {
    sdMeasuredLogFile.write(&measuredHeader, sizeof(measuredHeader));
    sdDerivedLogFile.write(&derivedHeader, sizeof(derivedHeader));
    sdCellLogFile.write(&cellHeader, sizeof(cellHeader));
    sdMeasuredLogFile.flush();
    sdDerivedLogFile.flush();
    sdCellLogFile.flush();
    sdLogActive = true;
    Serial.print("[Log] Measured data: ");
    Serial.println(measuredName);
    Serial.print("[Log] Derived data:  ");
    Serial.println(derivedName);
    Serial.print("[Log] Per-cell data: ");
    Serial.println(cellName);
  } else {
    Serial.println("[Log] Failed to create all session logs; logging disabled");
    if (sdMeasuredLogFile) sdMeasuredLogFile.close();
    if (sdDerivedLogFile) sdDerivedLogFile.close();
    if (sdCellLogFile) sdCellLogFile.close();
  }
}

// One paired log per power cycle. Legacy LOG files are included so session
// numbers are never reused on a card containing logs from older firmware.
uint32_t nextLogSessionNumber() {
...
uint32_t maxLogSessionInDir(File dir) {
  ...
  for (File entry = dir.openNextFile(); entry; entry = dir.openNextFile()) {
    ...
    bool isSessionLog = sscanf(entry.name(), "MEAS%u.BIN", &session) == 1 ||
                        sscanf(entry.name(), "DERI%u.BIN", &session) == 1 ||
                        sscanf(entry.name(), "CELL%u.BIN", &session) == 1 ||
                        sscanf(entry.name(), "LOG%u.BIN", &session) == 1;
    if (isSessionLog && (uint32_t)session > maxSession) {
      maxSession = session;
    }
    entry.close();
  }
...
void writeFastLogRecord(uint32_t nowMs) {
  ...
  MeasuredFastRecord measured;
  measured.type = (float)LOG_RECORD_FAST;
  measured.timeSeconds = (float)nowMs / 1000.0f;
  measured.apps1AdcCounts = (float)rawApps1Last;
  measured.apps2AdcCounts = (float)rawApps2Last;
  measured.bse1AdcCounts = (float)rawBse1Last;
  measured.bse2AdcCounts = (float)rawBse2Last;
  measured.hvCurrentAdcCounts = (float)rawHvCurrentLast;
  measured.motorRpm = (float)motorRpm;
  measured.busVoltageV = inverterDcBusVoltage;
  measured.busCurrentA = inverterDcBusCurrent;
  appendSdLog(sdMeasuredLogFile, true, &measured, sizeof(measured));

  DerivedFastRecord derived;
  derived.type = (float)LOG_RECORD_FAST;
  derived.timeSeconds = (float)nowMs / 1000.0f;
  derived.brakePressurePct = brakePressurePct;
  derived.commandedTorqueNm = commandedTorqueNm;
  derived.regenTorqueNm = regenTorqueNm;
  derived.speedKmh = speedKmh;
  derived.cumulativeDistanceKm = cumulativeDistanceKm;
  derived.statusFlags = (float)logStatusFlags();
  appendSdLog(sdDerivedLogFile, false, &derived, sizeof(derived));
}

void writeMediumLogRecord(uint32_t nowMs) {
  MeasuredMediumRecord measured;
  measured.type = (float)LOG_RECORD_MEDIUM;
  measured.timeSeconds = (float)nowMs / 1000.0f;
  measured.glvVoltageV = haveGlvVoltage ? glvVoltage : NAN;
  measured.inverterTempC = haveInverterTemp ? (float)inverterTempTenthsC / 10.0f : NAN;
  measured.motorTempC = havePm100MotorTemp ? (float)pm100MotorTempTenthsC / 10.0f : NAN;
  measured.socPct = haveBmsStatus ? (float)stateOfChargePct : NAN;
  measured.batteryTempC = haveCellTempStats ? (float)batteryTempC : NAN;
  measured.bmsFlags = (float)bmsFlags;
  measured.bmsFaultCode = haveBms2026FaultCode ? (float)bms2026FaultCode : NAN;
  measured.imdWarnAlarms = (float)imdWarnAlarms;
  measured.pm100PostFaults = (float)pm100PostFaults;
  measured.pm100RunFaults = (float)pm100RunFaults;
  appendSdLog(sdMeasuredLogFile, true, &measured, sizeof(measured));

  DerivedMediumRecord derived;
  derived.type = (float)LOG_RECORD_MEDIUM;
  derived.timeSeconds = (float)nowMs / 1000.0f;
  derived.apps1Pct = apps1Pct;
  derived.apps2Pct = apps2Pct;
  derived.pedalPct = pedalPct;
  derived.desiredTorqueNm = desiredTorqueNm;
  derived.powerKw = powerKw;
  derived.speedMph = speedMph;
  derived.fanDutyPct = (float)constrain(fan1DutyPercent, 0, 100);
  derived.rtdState = (float)rtdState;
  appendSdLog(sdDerivedLogFile, false, &derived, sizeof(derived));
}

void writeSlowLogRecord(uint32_t nowMs) {
  MeasuredSlowRecord measured;
  measured.type = (float)LOG_RECORD_SLOW;
  measured.timeSeconds = (float)nowMs / 1000.0f;
  measured.minCellVoltageV = haveCellVoltageStats ? (float)minCellVoltageMv / 1000.0f : NAN;
  measured.maxCellVoltageV = haveCellVoltageStats ? (float)maxCellVoltageMv / 1000.0f : NAN;
  measured.avgCellVoltageV = haveCellVoltageStats ? (float)avgCellVoltageMv / 1000.0f : NAN;
  measured.minCellTempC = haveCellTempStats ? (float)minCellTempC : NAN;
  measured.maxCellTempC = haveCellTempStats ? (float)maxCellTempC : NAN;
  measured.avgCellTempC = haveCellTempStats ? (float)avgCellTempC : NAN;
  measured.imdRIsoKohm = (imdRIsoKohm != IMD_SNV_WORD) ? (float)imdRIsoKohm : NAN;
  measured.imdRIsoStatus = (float)imdRIsoStatus;
  measured.validFlags = (float)((haveCellVoltageStats ? 0x01 : 0) |
...
void writeImuLogRecord(uint32_t nowMs) {
  ...
  MeasuredImuRecord measured;
  measured.type = (float)LOG_RECORD_IMU;
  measured.timeSeconds = (float)nowMs / 1000.0f;
  measured.accelXG = (float)imuAccelRaw[0] / 8192.0f;
  measured.accelYG = (float)imuAccelRaw[1] / 8192.0f;
  measured.accelZG = (float)imuAccelRaw[2] / 8192.0f;
  measured.gyroXDps = (float)imuGyroRaw[0] / 65.5f;
  measured.gyroYDps = (float)imuGyroRaw[1] / 65.5f;
  measured.gyroZDps = (float)imuGyroRaw[2] / 65.5f;
  measured.temperatureC = (float)imuTempRaw / 340.0f + 36.53f;
  appendSdLog(sdMeasuredLogFile, true, &measured, sizeof(measured));
}
...
void appendSdLog(File &file, bool measuredLog, const void *data, size_t len) {
  ...
  uint8_t *buffer = measuredLog ? measuredLogBuffer : derivedLogBuffer;
  size_t &bufferLen = measuredLog ? measuredLogBufferLen : derivedLogBufferLen;
  if (bufferLen + len > 512) {
    flushSdLogBuffer(file, measuredLog);
    if (!sdLogActive) {
      return;
    }
  }
...
void flushSdLogBuffer(File &file, bool measuredLog) {
  uint8_t *buffer = measuredLog ? measuredLogBuffer : derivedLogBuffer;
  size_t &bufferLen = measuredLog ? measuredLogBufferLen : derivedLogBufferLen;
  if (!sdLogActive || bufferLen == 0) {
    bufferLen = 0;
    return;
  }

  size_t written = file.write(buffer, bufferLen);
  if (written != bufferLen) {
    Serial.println("[Log] SD write failed; SD logging stopped");
    sdMeasuredLogFile.close();
    sdDerivedLogFile.close();
    sdCellLogFile.close();
    sdLogActive = false;
    screenDirty = true;
  }
...
void appendCellLog(const void *data, size_t len) {
  if (!sdLogActive) return;
  if (cellLogBufferLen + len > sizeof(cellLogBuffer)) {
    flushCellLogBuffer();
    if (!sdLogActive) return;
  }
  memcpy(cellLogBuffer + cellLogBufferLen, data, len);
  cellLogBufferLen += len;
}

void flushCellLogBuffer() {
  if (!sdLogActive || cellLogBufferLen == 0) {
    cellLogBufferLen = 0;
    return;
  }
  if (sdCellLogFile.write(cellLogBuffer, cellLogBufferLen) != cellLogBufferLen) {
    Serial.println("[Log] SD write failed; SD logging stopped");
    sdMeasuredLogFile.close();
    sdDerivedLogFile.close();
    sdCellLogFile.close();
...
void flushAllSdLogBuffers() {
  flushSdLogBuffer(sdMeasuredLogFile, true);
  flushSdLogBuffer(sdDerivedLogFile, false);
  flushCellLogBuffer();
}
...
// Hex-dumps all files in a session over serial so logs can be exported
...
void dumpLogFile(uint32_t session) {
  ...
  const char *prefixes[3] = {"MEAS", "DERI", "CELL"};
  for (uint8_t fileIndex = 0; fileIndex < 3; fileIndex++) {
    char name[16];
    snprintf(name, sizeof(name), "%s%04lu.BIN", prefixes[fileIndex],
    ...
    while ((n = f.read(buf, sizeof(buf))) > 0) {
      for (int i = 0; i < n; i++) printCanHexByte(buf[i]);
      Serial.println();
    }
    f.close();
...
void drawDashboard() {
  ...
  static char lastDistance[20] = ""; static uint16_t lastDistanceColor = ST7735_WHITE;
  ...
  static char lastPedal[24] = "";   static uint16_t lastPedalColor = ST7735_WHITE;
  ...
  static char lastSoc[24] = "";     static uint16_t lastSocColor = ST7735_WHITE;
  ...
  static char lastSd[24] = "";      static uint16_t lastSdColor = ST7735_WHITE;
  ...
  // ---- Header: SD/session status and power-derate banner ----
  uint16_t sdColor;
  if (!sdCardStarted) {
    snprintf(value, sizeof(value), "SD:NO LOG:----");
    sdColor = ST7735_RED;
  } else {
    snprintf(value, sizeof(value), "SD:%s LOG:%04lu", sdLogActive ? "OK" : "ERR",
             (unsigned long)logSessionNumber);
    sdColor = sdLogActive ? ST7735_GREEN : ST7735_YELLOW;
  }
  drawCachedDashboardText(100, 5, 196, 18, 1, value, sdColor,
                          lastSd, sizeof(lastSd), lastSdColor, force);
  drawDerateBanner(force);

  // ---- Zone B: large primary tiles (value cell = tileX+6, tileY+34) ----
  snprintf(value, sizeof(value), "%.0f", speedMph);
  drawCachedDashboardText(8, 66, 144, 48, 5, value, ST7735_WHITE,
                          lastSpeed, sizeof(lastSpeed), lastSpeedColor, force);

  snprintf(value, sizeof(value), "RUN:%.2fkm", cumulativeDistanceKm);
  drawCachedDashboardText(8, 118, 144, 10, 1, value, ST7735_WHITE,
                          lastDistance, sizeof(lastDistance), lastDistanceColor, force);
  ...
  // r0: pedal/brake inputs, signed torque request, power, SOC/regen setting.
  // These pedal-derived diagnostics remain live while TS/RTD is off.
  snprintf(value, sizeof(value), "PED:%.0f%% BRK:%.0f%%", pedalPct, brakePressurePct);
  ...
  snprintf(value, sizeof(value), "CMD:%+.1fNm", displayedCommandTorqueNm);
  ...
  snprintf(value, sizeof(value), "SOC:%u%% REGEN:%s", (unsigned int)stateOfChargePct,
           REGEN_ENABLED ? "ON" : "OFF");
  drawCachedDashboardText(dc3, r0, dw, dh, 1, value,
                          REGEN_ENABLED ? ST7735_GREEN : ST7735_YELLOW,
                          lastSoc, sizeof(lastSoc), lastSocColor, force);

  // r1: RPM, BMS status, inverter status, contactor
  snprintf(value, sizeof(value), "RPM:%.0f", (float)motorRpm);
...
// Request one complete 96-cell voltage dump and one temperature dump each
// second. The delay keeps the two BMS response sequences from overlapping.
void serviceCellDataRequests(uint32_t nowMs) {
  if (!sdLogActive || !haveBmsStatus || bmsTimedOut) return;

  if (cellTemperatureRequestPending) {
    if (nowMs - cellVoltageRequestMs < CELL_VOLTAGE_DUMP_WAIT_MS) return;

    CAN_message_t request{};
    request.id = BMS_CELL_REQUEST_2026_ID;
    request.len = 1;
    request.buf[0] = CELL_DATA_REQUEST_TEMPERATURES;
    sendCanFrame(request);
    cellTemperatureRequestPending = false;
    return;
  }

  if (nowMs - lastCellLogRequestMs < CELL_LOG_REQUEST_PERIOD_MS) return;

  CAN_message_t request{};
  request.id = BMS_CELL_REQUEST_2026_ID;
  request.len = 1;
  request.buf[0] = CELL_DATA_REQUEST_VOLTAGES;
  sendCanFrame(request);
  lastCellLogRequestMs = nowMs;
  cellVoltageRequestMs = nowMs;
  cellTemperatureRequestPending = true;
}

void writeCellDataResponseLog(const CAN_message_t &msg) {
  if (!sdLogActive || msg.flags.extended || msg.len < 2) return;

  const uint16_t frameIndex = msg.buf[0];
  const float timeSeconds = (float)millis() / 1000.0f;

  if (lastCellDataRequestType == CELL_DATA_REQUEST_VOLTAGES) {
    for (uint8_t i = 0; i < 3 && (1 + i * 2 + 1) < msg.len; i++) {
      const uint16_t zeroBasedCell = frameIndex * 3u + i;
      if (zeroBasedCell >= BMS_CELL_COUNT) break;
      const uint16_t millivolts = u16Be(msg.buf[1 + i * 2], msg.buf[2 + i * 2]);
      CellLogRecord record = {
        (float)CELL_DATA_REQUEST_VOLTAGES, timeSeconds, (float)(zeroBasedCell + 1u),
        millivolts == 0xFFFF ? NAN : (float)millivolts / 1000.0f
      };
      appendCellLog(&record, sizeof(record));
    }
  } else if (lastCellDataRequestType == CELL_DATA_REQUEST_TEMPERATURES) {
    for (uint8_t i = 0; i < 7 && (1 + i) < msg.len; i++) {
      const uint16_t zeroBasedCell = frameIndex * 7u + i;
      if (zeroBasedCell >= BMS_CELL_COUNT) break;
      const uint8_t rawTemperature = msg.buf[1 + i];
      CellLogRecord record = {
        (float)CELL_DATA_REQUEST_TEMPERATURES, timeSeconds, (float)(zeroBasedCell + 1u),
        rawTemperature == 0xFF ? NAN : (float)((int)rawTemperature - 40)
      };
      appendCellLog(&record, sizeof(record));
    }
  }
...
void dispatchReceivedCanFrame(const CAN_message_t &msg) {
  ...
  switch (msg.id) {
    ...
    case BMS_CELL_RESPONSE_2026_ID:
      writeCellDataResponseLog(msg);
      break;
...
void handleBms2026StatusFrame(const CAN_message_t &msg) {
  ...
  bmsTimedOut      = false;
  lastBmsRxMs      = nowMs;
  lastBmsCellVoltageRxMs = nowMs;
  // Remain fail-safe until the following 0x311 frame supplies the matching
  // current measurement and the pair passes validation.
  regenBmsPairValid = false;
  bmsFlags         = flags;
...
void handleBms2026TempFrame(const CAN_message_t &msg) {
  ...
  haveBmsCurrent = true;
  uint32_t nowMs = millis();
  lastBmsCurrentRxMs = nowMs;

  // The BMS sends 0x310 immediately before 0x311. Pair only adjacent/fresh
  // frames so a current sample is never used to correct an unrelated voltage.
  regenBmsPairValid = false;
  if (haveCellVoltageStats &&
      (uint32_t)(nowMs - lastBmsCellVoltageRxMs) <=
          REGEN_BMS_PAIR_MAX_SKEW_MS) {
    regenBmsPairValid = pushRegenVoltageSample(nowMs);
  }
...
void updatePm100CommandState() {
  ...
  // Calculate the diagnostic request before the TS/RTD safety gate. This lets
  // pedal, brake, and regen commissioning be observed with HV disabled while
  // the actual PM100 command below remains zero and disabled.
  displayedCommandTorqueNm = calculateDisplayedCommandTorqueNm();
  ...
  if (torqueAllowed) {
    torqueRaw = (int16_t)roundf(displayedCommandTorqueNm * (float)TORQUE_SCALE);
...
// Builds the signed, quantized torque request independently of TS/RTD state.
// Safety inputs still affect the request through updatePedal and
// calculateRegenTorqueNm; updatePm100CommandState separately decides whether
// this value is permitted onto the CAN bus.
float calculateDisplayedCommandTorqueNm() {
  float torqueNm = desiredTorqueNm;
  regenTorqueNm = calculateRegenTorqueNm();
  if (regenTorqueNm > 0.0f) {
    // Negative torque in the selected forward direction commands regeneration.
    torqueNm = -regenTorqueNm;
  }

  if (powerDerateActive && torqueNm > 0.0f) {
    float cap = powerDerateTorqueCapNm(motorRpm);
    if (torqueNm > cap) torqueNm = cap;
  }

  return roundf(torqueNm * (float)TORQUE_SCALE) / (float)TORQUE_SCALE;
}
...
void updateBrakePressureSensors() {
  ...
  brakePressurePct = max(bse1Pct, bse2Pct);
}

// Returns a positive regen-torque magnitude. The caller applies it as negative
// PM100 torque. Mechanical braking remains fully independent of this request.
float calculateRegenTorqueNm() {
  if (!REGEN_ENABLED || !braking || !haveBmsStatus || !appsPlausible ||
      bspcActive || pedalPct > BSPC_APPS_RESET_PCT ||
      speedKmh <= REGEN_CUTOFF_SPEED_KMH) {
    return 0.0f;
  }

  // No fresh, synchronized BMS voltage/current pair means no safe basis for a
  // charging-current limit, so regen fails off.
  float cellChargeCurrentLimitA = regenCellChargeCurrentLimitA(millis());
  if (cellChargeCurrentLimitA <= 0.0f) return 0.0f;

  // Triangular pressure curve: build regen through light braking, then taper
  // it away as the driver enters the heavy mechanical-braking region.
  float pressureFactor;
  if (brakePressurePct <= REGEN_LIGHT_BRAKE_END_PCT) {
    pressureFactor = brakePressurePct / REGEN_LIGHT_BRAKE_END_PCT;
  } else if (brakePressurePct < REGEN_HEAVY_BRAKE_END_PCT) {
    pressureFactor = (REGEN_HEAVY_BRAKE_END_PCT - brakePressurePct) /
                     (REGEN_HEAVY_BRAKE_END_PCT - REGEN_LIGHT_BRAKE_END_PCT);
  } else {
    pressureFactor = 0.0f;
  }

  // Fade smoothly from full regen at 15 km/h to zero at the 5 km/h cutoff.
  float speedFactor = constrain((speedKmh - REGEN_CUTOFF_SPEED_KMH) /
                                (REGEN_FULL_SPEED_KMH - REGEN_CUTOFF_SPEED_KMH),
                                0.0f, 1.0f);

  // Linearly reduce the allowed charging power from 20 kW at 90% SOC to zero
  // at 95% SOC. The resulting torque cap enforces P = torque*rpm/9549.
  float socFactor = constrain((REGEN_SOC_DISABLE_PCT - (float)stateOfChargePct) /
                              (REGEN_SOC_DISABLE_PCT - REGEN_SOC_DERATE_START_PCT),
                              0.0f, 1.0f);
  float torqueNm = REGEN_MAX_TORQUE_NM * pressureFactor * speedFactor;
  float absRpm = fabsf((float)motorRpm);
  if (absRpm > 1.0f) {
    // Use min-cell voltage times the 95 active series banks as a lower bound
    // on pack voltage. With unity conversion efficiency, this makes the
    // mechanical-power-to-current conversion conservative.
    float packVoltageLowerBoundV =
        ((float)minCellVoltageMv / 1000.0f) *
        (float)REGEN_PACK_SERIES_BANKS;
    if (packVoltageLowerBoundV <= 0.0f) return 0.0f;

    float regenPowerLimitKw = REGEN_MAX_POWER_KW * socFactor;
    float cellVoltagePowerLimitKw =
        packVoltageLowerBoundV * cellChargeCurrentLimitA / 1000.0f;
    if (cellVoltagePowerLimitKw < regenPowerLimitKw) {
      regenPowerLimitKw = cellVoltagePowerLimitKw;
    }

    float powerTorqueCapNm = TORQUE_PER_KW_PER_RPM *
                             regenPowerLimitKw / absRpm;
    if (torqueNm > powerTorqueCapNm) torqueNm = powerTorqueCapNm;
  }
  return constrain(torqueNm, 0.0f, REGEN_MAX_TORQUE_NM);
}

// Integrate the RPM-derived road speed. This is intentionally approximate:
// MPH_PER_RPM already embodies the vehicle's gearing and tire circumference.
void updateCumulativeDistance(uint32_t nowMs) {
  if (lastDistanceUpdateMs != 0) {
    float elapsedHours = (float)(nowMs - lastDistanceUpdateMs) / 3600000.0f;
    cumulativeDistanceKm += speedKmh * elapsedHours;
  }
